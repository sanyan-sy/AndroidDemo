...
/**
 * 非绑定式服务（启动服务之后，活动与服务基本就没什么关系了）
 * 在活动里调用了startService()方法来启动DownloadService，然后DownloadService的onCreate()和onStartConmmand()
 * 就会执行，之后服务会一直处于运行状态，但具体运行的是什么逻辑，活动就控制不了了。
 * （即，活动并不知道服务到底去做什么以及完成的如何）
 */
...
//    private static final String SAVE_PATH = "/path/to/save/file.zip";  // 文件保存路径
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        Log.d("Service", "-----启动服务：onStartCommand-----");
        startDownload();
